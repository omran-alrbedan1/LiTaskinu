...
import { Plus, Edit, User, Image as ImageIcon, Briefcase, GraduationCap } from "lucide-react";
import { ICONS } from "@/constants/icons";
import CustomUpload from "@/components/shared/CustomUpload";
import { PsychiatristFormValidation } from "@/validation/admin";

interface PsychiatristFormProps {
  onSubmit: (data: Psychiatrist) => void;
  onCancel: () => void;
  isLoading?: boolean;
  initialData?: Psychiatrist | null;
  isEdit?: boolean;
}

export function PsychiatristForm({
  onSubmit,
  onCancel,
  isLoading = false,
  initialData,
  ...
  const form = useForm<z.infer<typeof PsychiatristFormValidation>>({
    resolver: zodResolver(PsychiatristFormValidation),
    defaultValues: {
      name: "",
      email: "",
      username: "",
  ...
  const handleSubmit = (values: z.infer<typeof PsychiatristFormValidation>) => {
    const psychiatristData: Psychiatrist = {
      ...
      phone: values.phone,
      image: imagePreview || "/placeholder-avatar.jpg",
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(handleSubmit)} className="space-y-6">
        ...
        <Card className="border-purple-200 dark:border-purple-800 bg-purple-50/20 dark:bg-purple-950/20">
          <CardHeader>
            <div className="flex items-center gap-2">
              <div className="flex items-center justify-center w-8 h-8 bg-purple-100 dark:bg-purple-900 rounded-md">
                <Briefcase className="text-purple-600 dark:text-purple-400 size-4" />
              </div>
              <div>
                <CardTitle className="text-gray-900 dark:text-gray-100">
          ...
          <CardContent className="space-y-4">
            ...
            <CustomFormField
              ...
              name="experience"
              label="Experience"
              placeholder="e.g., 10 years of clinical practice, specialized in cognitive behavioral therapy..."
            />
            ...
            <CustomFormField
              ...
              tagInputProps={{
                placeholder: "Add specialization (e.g., Depression, Anxiety, PTSD)",
        ...
        <Card className="border-green-200 dark:border-green-800 bg-green-50/20 dark:bg-green-950/20">
          <CardHeader>
            <div className="flex items-center gap-2">
              <div className="flex items-center justify-center w-8 h-8 bg-green-100 dark:bg-green-900 rounded-md">
                <GraduationCap className="text-green-600 dark:text-green-400 size-4" />
              </div>
              <div>
                <CardTitle className="text-gray-900 dark:text-gray-100">
                ...
                <CardDescription className="text-gray-600 dark:text-gray-400">
                  Detailed background and professional credentials
                </CardDescription>
              </div>
            </div>
          </CardHeader>
          <CardContent>
            <CustomFormField
              ...
              name="bio"
              label="Biography"
              placeholder="Write a detailed biography including education, certifications, approach to therapy, and notable achievements..."
            />
          </CardContent>
        </Card>
        ...
        <div className="flex justify-end gap-3 pt-4 border-t border-gray-200 dark:border-gray-800">
          ...
          >
            {isEdit ? "Update Psychiatrist" : "Add Psychiatrist"}
          </SubmitButton>
        </div>
      </form>
    </Form>
