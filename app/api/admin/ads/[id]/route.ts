...
export async function POST(request: NextRequest) {
  try {
    ...
    // Check if the path contains "change-status" followed by an ID
    const changeStatusIndex = pathSegments.indexOf("change-status");
    if (changeStatusIndex !== -1 && pathSegments.length > changeStatusIndex + 1) {
      const adId = pathSegments[changeStatusIndex + 1];
      return await handleStatusChange(request, session, API_BASE_URL, adId);
    }

    // Otherwise handle regular POST for creating ads
    const contentType = request.headers.get("content-type") || "";
...
async function handleStatusChange(
  ...
  try {
    ...
    // Optional: Validate status value if you have specific allowed values
    const allowedStatuses = ['active', 'inactive', 'pending', 'archived']; // Adjust as needed
    if (allowedStatuses.length > 0 && !allowedStatuses.includes(body.status)) {
      return NextResponse.json(
        {
          status: false,
          message: "Validation errors",
    ...
    const response = await axios.post(
      `${API_BASE_URL}/ads/change-status/${adId}`,
      { status: body.status },
      {
        headers: {
